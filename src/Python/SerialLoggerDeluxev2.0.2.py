...
class Cfg:
    ...
    KEY_SHOW_CTRL_CHARS = "show_control_chars"
...
def run_cli_mode(args):
    ...
    try:
        ...
        print(f"Error: Codificación '{config['encoding']}' no válida. {e}");
        return
    if args.log:
        try:
            logfile = open(args.log, 'a', encoding='utf-8');
            print(f"Registrando en: {args.log}")
        except IOError as e:
...
class SerialLoggerApp(tk.Tk):
    ...
    def _create_left_panel(self, parent):
        ...
        self.bt_clear = ttk.Button(port_frame, text="Limpiar", command=self._clear_output)
        ...
        self.ck_show_ctrl_chars = ttk.Checkbutton(output_header_frame, text="Ver Chars. Ctrl",
                                                  variable=self.show_control_chars_var)
    ...
    def _load_settings(self):
        ...
        self.show_control_chars_var.set(
            self.app_config.getboolean(Cfg.SEC_UI, Cfg.KEY_SHOW_CTRL_CHARS, fallback=True))
    ...
    def _save_settings(self):
        ...
        self.app_config.set(Cfg.SEC_UI, Cfg.KEY_SHOW_CTRL_CHARS,
                            str(self.show_control_chars_var.get()))
    ...
    # *** SOLUCIÓN 1 (RESTAURADA): _display_text para normalizar finales de línea en tiempo real ***
    def _display_text(self, raw_data: Optional[bytes], text_data: Optional[str], tag: str):
        ...
        if raw_data:
            if is_hex or self.protocol_selector_var.get() == "Modbus-RTU":
                ...
                decoded_text = codecs.getincrementaldecoder(self.encoding_var.get())(errors='replace').decode(raw_data)
                # Normaliza todos los tipos de salto de línea a \n para evitar artefactos
                display_string = decoded_text.replace('\r\n', '\n').replace('\r', '\n')
        elif text_data:  # Para datos enviados
        ...
        for char in display_string:
            ...
            # La opción de ver caracteres de control sigue funcionando
    ...
    def _set_defaults(self):
        ...
        self.show_control_chars_var.set(True)
    ...
    # *** SOLUCIÓN 2 (DEFINITIVA): update_port_list filtra por información de hardware ***
    def update_port_list(self):
        # Obtenemos la lista de objetos de puerto
        ports = serial.tools.list_ports.comports()

        # Filtramos la lista. Nos quedamos con los puertos que tienen fabricante o VID.
        # Esto elimina las consolas virtuales en Linux y es un método robusto y multiplataforma.
        port_names = [p.device for p in ports if p.manufacturer is not None or p.vid is not None]

        sorted_ports = sorted(port_names)
        current_selection = self.cb_commport.get()
        self.cb_commport['values'] = sorted_ports

        if sorted_ports:
            # Si la selección anterior sigue siendo válida, la mantenemos.
            if current_selection in sorted_ports:
                self.cb_commport.set(current_selection)
            # Si no, seleccionamos el primer puerto de la lista.
            else:
                self.cb_commport.set(sorted_ports[0])
        else:
            # Si no hay puertos, vaciamos la selección.
