...
# -----------------------------------------------------------------------------
# --- CLASE DE LÓGICA DE NEGOCIO (SIN GUI) ---
# -----------------------------------------------------------------------------
class SerialHandler:
    ...
    def open_port(self, config):
        ...
        try:
            ...
            # Si es "none", no hacemos nada, dejamos los valores por defecto de pySerial.
        ...
        try:
            while self.is_reading.is_set():
                try:
                    ...
                    if raw_chunk and self.output_callback:
                        self.output_callback(raw_chunk)
    ...
    def write_data(self, data, encoding='utf-8'):
        # ... (sin cambios)
        if self.serial_port and self.serial_port.is_open:
            try:
...
def run_cli_mode(args):
    ...
    config = {
        'port': args.port, 'baud': args.baud, 'databits': args.databits,
        'stopbits': args.stopbits, 'parity': args.parity, 'dtr': not args.no_dtr,
        'timestamp': args.timestamp, 'delimiter': ' ', 'encoding': args.encoding
    }
    ...
    try:
        # Usaremos este decodificador para toda la sesión
    ...
    def console_output(raw_chunk):
        ...
        # --- LÍNEA CORREGIDA: Usa el decodificador ya creado ---
    ...
    try:
        while handler.is_reading.is_set():
            time.sleep(1)
...
class SerialLoggerApp(tk.Tk):
    ...
    def _send_data(self, event=None):
        ...
        success, message = self.handler.write_data(full_data, self.encoding_var.get())
        if success:
    ...
    def _toggle_gui_elements(self, enabled):
        ...
        if not hasattr(self, 'control_widgets') or not hasattr(self, 'cb_commport'):
            self.control_widgets = [
                ...
                self.ck_logfile, self.tf_logfile, self.bt_fileselector, self.bt_open_port, self.ck_dtr, self.cb_encoding
            ]
        for widget in self.control_widgets:
            if isinstance(widget, ttk.Combobox):
                if 'readonly' in str(widget.cget('state')) or widget in (self.cb_stopbits, self.cb_databits,
                                                                         self.cb_parity, self.cb_handshake,
                                                                         self.cb_timestamp, self.cb_delimiter,
                                                                         self.cb_line_ending):
    ...
    def _load_settings(self):
        ...
        self.encoding_var.set(self.config.get("Serial", "encoding", fallback='utf-8'))
    ...
    def _save_settings(self):
        ...
        self.config.set('Serial', 'encoding', self.encoding_var.get());
    ...
    def open_port(self):
        self._clear_output()
        config = {
            'port': self.cb_commport.get(), 'baud': self.cb_baud.get(),
            'databits': self.cb_databits.get(), 'stopbits': self.cb_stopbits.get(),
            'parity': self.cb_parity.get(), 'handshake': self.cb_handshake.get(),
        ...
        if self.ck_logfile_var.get():
            ...
            if os.path.exists(log_filename):
                if not messagebox.askyesno("Archivo existente",
                                           f"El archivo '{log_filename}' ya existe. ¿Añadir datos?"):
                    self.print_log_flag = False
                else:
    ...
    def _process_gui_queue(self):
        try:
            while not self.gui_queue.empty():
                ...
                if self.ck_logfile_var.get() and self.print_log_flag:
...
if __name__ == "__main__":
    ...
    if args.port or args.no_gui:
        if not args.port:
            parser.error("--port es requerido para el modo CLI.")
        run_cli_mode(args)
    else:
        app = SerialLoggerApp()
        app.mainloop()